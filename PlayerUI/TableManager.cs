...
namespace QuanLyBanBida
{
    public partial class f_TableManager : Form
    {
        ...
        void LoadFoodListByCategoryID(int id)
        {
            ...
            comboBoxFood.DisplayMember = "NameAndCount";
        }
        ...
        private void SaveBillToPdf(int tableID, string billText)
        {
            ...
            document.Close();
            ViewerPDF f = new ViewerPDF(filePath); // form 1 của em sẽ xuất hiện tùy vào tài khoản đăng nhập
            f.ShowDialog();

            //openChildForm(new ViewerPDF(filePath));
            //hideSubMenu();
        }
        ...
        private void btn_SwitchTable_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show(string.Format("Bạn có thật sự muốn chuyển bàn {0} qua bàn {1}",  (cb_SwitchTable.SelectedItem as Table).Name, (lsvBill.Tag as Table).Name), "Thông báo", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
            {
        ...
        private void cb_SwitchTable_SelectedIndexChanged(object sender, EventArgs e)
        {
